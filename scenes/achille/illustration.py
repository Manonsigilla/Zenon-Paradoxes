...
class EtapeIllustration(SceneParadoxe):
    ...
    def x_tortue(self):
        if NB_INSTANTS == 0:
            return X_DEPART_TORTUE
        return X_DEPART_TORTUE + (self.k / NB_INSTANTS) * (X_ARRIVEE - X_DEPART_TORTUE)
    ...
    def _dessiner_coureurs(self, ecran):
        x_t = int(self.x_tortue())
        x_a = int(self.x_achille())

        # La tortue : un simple disque, en retrait derrière son avance
        pygame.draw.circle(ecran, theme.CREME, (x_t, Y_TRAJECTOIRE), 10)
        pygame.draw.circle(ecran, theme.SURFACE, (x_t, Y_TRAJECTOIRE), 10, width=1)

        # Achille : une pointe qui fonce, comme la flèche des autres étapes
        pygame.draw.polygon(ecran, theme.SURFACE,
                            [(x_a - 16, Y_TRAJECTOIRE - 8), (x_a + 2, Y_TRAJECTOIRE),
                            (x_a - 16, Y_TRAJECTOIRE + 8)])
        pygame.draw.polygon(ecran, theme.OR,
                            [(x_a - 16, Y_TRAJECTOIRE - 6), (x_a, Y_TRAJECTOIRE),
                            (x_a - 16, Y_TRAJECTOIRE + 6)])
    ...
    def _dessiner_textes(self, ecran):
        ...
        texte.dessiner_texte(
            ecran,
            "À cet instant précis, Achille n'a pas encore rejoint la tortue.",
            x, y + 52, theme.police(22), theme.ENCRE)
        texte.dessiner_texte(
            ecran,
            "Mais en passant à l'instant suivant, l'écart entre eux se réduit.",
            x, y + 84, theme.police(22), theme.ENCRE)
        if self.k == NB_INSTANTS:
            texte.dessiner_texte(
                ecran,
                "La succession des instants finit par les réunir au même endroit.",
