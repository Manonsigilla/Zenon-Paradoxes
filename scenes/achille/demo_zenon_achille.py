...
ETAPES = [
    ("La tortue part avec une avance sur Achille.", "D₀"),
    ("Achille court jusqu'au point de départ de la tortue.", "x = D₀"),
    ("Pendant ce temps, la tortue a avancé un peu plus loin.", "D₁ = D₀·r"),
    ("Achille atteint ce nouveau point, mais la tortue est déjà repartie.", "Dₙ₊₁ = Dₙ·r"),
    ("Un nouvel écart apparaît à chaque fois, une infinité de fois.", "Achille ne rattrape jamais ?"),
]
NUMERAUX = ["Α", "Β", "Γ", "Δ", "Ε"]
    ...
    def dessiner_contenu(self, ecran):
        ...
        if self.visibles == len(ETAPES):
            texte.dessiner_texte_centre(
                ecran,
                "Conclusion de Zénon : à chaque étape franchie, la tortue a "
                "déjà avancé ; Achille semblerait donc ne jamais la rattraper.",
